...
function email (email, allFields, chargeOptions = {}) {
  ...
  if (emailSuspiciousDigitVariation(email)) return 'containsTooManyDigits'
...
function emailSuspiciousDigitVariation (input) {
  if (!['string', 'number'].includes(typeof input)) return false
  const matchedDigits = String(input).match(/(\d)/g)
  return (matchedDigits !== null) && (matchedDigits.length > 15)
}
