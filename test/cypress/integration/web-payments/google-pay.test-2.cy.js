...
describe('Google Pay payment flow', () => {
  ...
  describe('Secure card payment page', () => {
    it.only('Should handle Google pay correctly', () => {
      ...
      cy.intercept(`/web-payments-auth-request/google/${chargeId}`, { times: 2 }, { forceNetworkError: true }).as('first-web-payments-auth-request-which-fails')

      // cy.intercept('POST', `/web-payments-auth-request/google/${chargeId}`, { forceNetworkError: true, times: 1 }).as('first-web-payments-auth-request-which-fails')
      // cy.intercept(
      //   { url: `/web-payments-auth-request/google/${chargeId}`, times: 1 },
      ...
      // cy.intercept(`/web-payments-auth-request/google/${chargeId}`, { method: 'POST', times: 2 }, { forceNetworkError: true }).as('first-request')
