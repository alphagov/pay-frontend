...
function storeTemplate (basePath) {
  return function partialStoreTemplate (templatePath, callback) {
    ...
    if (rawTemplates[templateName]) {
      logger.info('Compiling from stored template')
      compiledTemplates[templateName] = hogan.compile(rawTemplates[templateName])
      return callback(null)
    }
    fs.readFile(templatePath, {encoding: 'utf-8'}, function handlePartialStoreErr (err, data) {
      if (err) return callback(err)
      logger.info('Compiling from file system loaded template')
