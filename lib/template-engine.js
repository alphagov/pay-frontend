...
TemplateEngine._storeTemplate = function (basePath) {
    return function (templatePath, callback) {
        ...
        if (TemplateEngine.__rawTemplates[templateName]) {
            logger.error(`Using stored raw template for: ${templateName}`)
            TemplateEngine.__compiledTemplates[templateName] = hogan.compile(TemplateEngine.__rawTemplates[templateName])
            ...
            fs.readFile(templatePath, {encoding: 'utf-8'}, function (err, data) {
                ...
                logger.info(`Compiled ${templateName}!`)
                TemplateEngine.__rawTemplates[templateName] = data
                callback(null)
            })
        }
    }
